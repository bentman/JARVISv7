export function createApiClient(invoke) {
  ...
  return {
    ...
    startResidentVoiceStream: async () => JSON.parse(await invoke("start_resident_voice_stream")),
    stopResidentVoiceStream: async () => JSON.parse(await invoke("stop_resident_voice_stream")),
    ...
    setResidentVoiceMode: async (mode) => JSON.parse(await invoke("set_resident_voice_mode", { mode })),
