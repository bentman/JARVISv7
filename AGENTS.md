...
## 2. Before changing config or dependencies
...
- Approved dependency changes go through the right `pyproject.toml` group or hardware extra, then `scripts/provision.py`; direct `pip install` is not repo evidence.
- Use `>=` for dependency versions by default; use `==` only with a documented reason.
...
- Do not add environment variables, dependency groups, setup paths, or storage roots unless they are the narrowest approved solution.
...
## 4. Work in the order an agent should encounter it
...
Use deterministic, non-interactive commands with explicit paths. Prefer existing files, patterns, helpers, and tests. Do not introduce parallel architectures, shadow workflows, alternate setup paths, custom helper scripts, or new docs unless explicitly required.
...
- host class, such as `windows-amd64` or `windows-arm64`
