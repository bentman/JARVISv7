...
- 2026-05-03 16:24
  ...
  - Host class(es): Windows x64, Windows ARM64
  - Evidence: x64 report evidence (`reports/validation/slice_h/h0-x64.md`) records `backend\.venv\Scripts\python scripts\validate_backend.py profile` PASS with `arch=amd64`, Python `3.12.10`, extras `[hw-cpu-base, hw-x64-base, hw-gpu-nvidia-cuda, dev]`, readiness `ready`, CPU EP present, CUDA EP not observed, DirectML EP not observed, QNN EP/`QnnHtp` not observed, CUDA DLL discovery paths present; and `backend\.venv\Scripts\python scripts\validate_backend.py regression` PASS with `96 passed`. ARM64 report evidence (`reports/validation/slice_h/h0-arm64.md`) records `backend\.venv\Scripts\python scripts\validate_backend.py profile` PASS with `arch=arm64`, Python `3.12.10`, extras `[hw-cpu-base, hw-arm64-base, hw-npu-qualcomm-qnn, dev]`, readiness `ready`, CPU EP present, QNN EP missing, `QnnHtp` missing, DirectML EP not observed, CUDA path not host-applicable; and `backend\.venv\Scripts\python scripts\validate_backend.py regression` PASS with `96 passed`.
  - Note: Combined H.0 close-state matrix — STT CPU: x64 `PASS`, ARM64 `PASS`; TTS CPU: x64 `PASS`, ARM64 `PASS`; STT CUDA: x64 `SKIP-prereq-missing`, ARM64 `SKIP-no-host`; TTS CUDA: x64 `SKIP-prereq-missing`, ARM64 `SKIP-no-host`; STT QNN: x64 `SKIP-no-host`, ARM64 `SKIP-prereq-missing`; TTS QNN: x64 `SKIP-no-host`, ARM64 `SKIP-prereq-missing`; STT DirectML: x64 `SKIP-prereq-missing`, ARM64 `SKIP-prereq-missing`; TTS DirectML: x64 `SKIP-prereq-missing`, ARM64 `SKIP-prereq-missing`. H.0 remained census/proof only: no source/test/config/model/package/slice-doc/inventory/generated-file mutations, no CUDA/QNN/DirectML activation attempts, no STT/TTS runtime behavior changes, and no model export/download/mutation. CPU voice paths remain fallback authority for H.1; H.2/H.3, H.4, H.5/H.6, and H.7 remain forward prerequisite/activation work.
