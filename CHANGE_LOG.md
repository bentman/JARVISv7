...
- 2026-06-18 02:49
  - Summary: Corrected Slice R local LLM serve-profile resolution so current-host accelerator profiles are selected when complete runtime evidence exists, instead of always selecting CPU. ARM64 QNN now falls back to CPU when QNN evidence is incomplete and selects the QNN profile when hardware/preflight, model, and sidecar-binary evidence are present.
  - Scope: `backend/app/models/llm_profiles.py`, `backend/tests/unit/runtimes/llm/test_llm_serve_profiles.py`
  - Host class(es): Windows ARM64 / arm64 validated. Windows AMD64 validation pending for the same code path.
  - Evidence: `backend\.venv\Scripts\python -m pytest backend\tests\unit\runtimes\llm\test_llm_serve_profiles.py backend\tests\unit\services\test_local_llm_sidecar.py -q` PASS (`27 passed`). `backend\.venv\Scripts\python scripts\validate_backend.py unit` PASS (`539 passed`, one existing Starlette warning). `backend\.venv\Scripts\python scripts\validate_backend.py regression` PASS (`119 passed, 4 deselected`, report `reports\validation\20260618024925-regression.txt`; fingerprint `arch=arm64 python=3.13.13 extras=[hw-cpu-base,hw-arm64-base,hw-npu-qualcomm-qnn,dev] readiness=ready`). `git diff --check` PASS with line-ending warnings only.
