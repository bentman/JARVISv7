...
Model catalogs may define host-class profiles before validation hardware is available. Such entries must be explicit about their state, using labels such as `validated`, `declared-not-validated`, or `declared-degraded`. Defined-but-unvalidated wiring is allowed when it captures researched configuration, provisioning shape, readiness behavior, fallback behavior, and deterministic skip/degraded reasons. `config/models/llm.yaml`, `stt.yaml`, `tts.yaml`, and `wake.yaml` carry `validation_status` on their model, device, or serve-profile entries. CPU, CUDA, DirectML, Windows ARM64 QNN/NPU, and Windows ARM64 Adreno OpenCL paths with live proof are `validated`; paths that are researched but not live-proven remain declared with their degraded or not-validated state. The llama.cpp QNN path remains operator opt-in because Windows Hexagon HTP loading requires test-signing on current Copilot+ hardware.
