...
- route/API contract
- runtime proof path
- known host class when relevant

## Follow-up

Only gaps required to finish this ADR while status is `Accepted`.

Once status is `Implemented`, this section should be empty or contain links to separate follow-up ADRs/backlog items. It should not hold general roadmap.
