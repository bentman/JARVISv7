...
A capability whose external counterpart is itself a session - a still-running subprocess, a resumable remote conversation - shares one host-owned session-lifecycle mechanism instead of each capability family managing its own process lifetime. The mechanism keeps a session's state reachable across separate calls; it does not by itself decide whether that state lives only in a still-running process or can also be recovered from durable storage after the process is gone - that choice belongs to whichever family's own protocol requirements demand it. This is an execution mechanism, not a new effect class: a session-bearing capability still declares whatever `external_read`, `external_write`, or `privileged_execution` its own operation warrants.
...
Shared session-lifecycle mechanism for external capability connections - not yet built, and the concrete reason two existing capability families cannot yet keep a connection open across calls:
- `_mcp` in `backend/app/services/extension_runtime_service.py` and `run_acp` in `backend/app/extensions/acp.py` are the only two `asyncio.run()`-per-call sites in the backend. Each opens an external subprocess/connection, does one exchange, and tears it down inside that same call (`_mcp`'s `finally: await runtime.close()`; `run_acp`'s `finally: await asyncio.to_thread(_stop_process_tree, process.pid)`), so no session state survives past one operation. `asyncio.run()` creates and destroys its own event loop every call, and the subprocess transport/streams a persistent session would need to hold are bound to the loop that created them - they cannot survive past that call returning, so this is not a small tweak to either function.
- For MCP `stdio`, that contradicts the transport's own design: a client keeps one shared channel open across operations - "there are no per-request streams" - and only restarts it after unexpected termination, retrying the lost in-flight request rather than reopening on a schedule (`https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/stdio`).
- For ACP, that forecloses calling `session/prompt` more than once against the same session, and forecloses `session/load` (replays history) and `session/resume` (does not) ever being reachable, since both require a session whose state the agent can still reach (`https://github.com/agentclientprotocol/rust-sdk/blob/main/md/protocol-v2.md`). Neither is universally required: an agent advertises `loadSession`/`sessionCapabilities.resume` support at `initialize`, and a client that never needs a second prompt in the same session has nothing to attach this mechanism to - it is opt-in per family, the same way the protocol itself treats it as a negotiated capability, not a mandatory one.
- The mechanism must not assume the resource it opened stays alive forever: MCP's own guidance is to restart a stdio server that exits unexpectedly and retry the lost request, and ACP's own production guidance is durable session storage and recovery across process failure, not a process that must never crash (`https://github.com/agentclientprotocol/rust-sdk/blob/main/md/protocol-v2-quickstart.md`). It must report a dead resource and support recreating one under the same session identity rather than only detecting death after a call fails against it.
- Shape: a background event loop (or a dedicated thread running one) owns the long-lived resource; a call is dispatched into that loop instead of starting its own; an explicit close ends the session deliberately (definition edit/delete/disable, backend shutdown) rather than only via the `finally` block on whichever call happened to open it.
- Belongs alongside `backend/app/actions/boundaries.py`'s other shared execution primitives (`ProcessBoundary`, `ExecutionBoundary`, `run_bounded`, cancellation) - the same tier already declaring the process/root isolation MCP, ACP, tool, and skill-script capabilities register through, not a new one.
- Attachment points once built: ADR 0006 attaches it for MCP `stdio` connection reuse (discover/tool/resource/prompt operations against one open connection instead of one process per operation); ADR 0007 attaches it for ACP outbound session reuse (repeated `session/prompt` against one session, then `session/load`/`session/resume`). Neither family's ADR should build its own bespoke persistence layer once this exists.
