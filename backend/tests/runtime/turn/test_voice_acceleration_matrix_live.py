...
def test_voice_acceleration_matrix_live_cpu_turn_passes_current_host() -> None:
    engine = TurnEngine(
        stt=OnnxWhisperRuntime(device="cpu"),
        tts=NullTTSRuntime(reason="TTS acceleration matrix live gate uses STT+LLM full-turn proof only"),
