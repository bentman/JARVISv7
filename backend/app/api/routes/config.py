...
OPERATOR_FIELD_SPECS: tuple[OperatorFieldSpec, ...] = (
    OperatorFieldSpec("USE_LOCAL_MODEL", "Enable local model runtime selection.", section="Model"),
    OperatorFieldSpec(
        "LLM_MODEL_POLICY",
        "Model policy.",
    ...
    OperatorFieldSpec("USE_OLLAMA", "Enable the local Ollama runtime.", section="Model"),
    OperatorFieldSpec("OLLAMA_MODEL", "Ollama model name.", section="Model"),
    OperatorFieldSpec("LOCAL_MODEL_FETCH", "Allow local model fetch behavior.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_MODEL_PATH", "Local llama.cpp model path.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_BASE_URL", "Managed llama.cpp endpoint URL.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_HOST", "Managed llama.cpp bind host.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_PORT", "Managed llama.cpp bind port.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_BINARY_PATH", "Managed llama.cpp server binary override.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_MANAGED", "Enable managed llama.cpp sidecar launch.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_MODEL_NAME", "Managed llama.cpp logical model name.", section="Model", advanced=True),
    OperatorFieldSpec("LLAMA_CPP_TIMEOUT_SECONDS", "Managed llama.cpp HTTP timeout in seconds.", section="Model", advanced=True),
    OperatorFieldSpec("OLLAMA_BASE_URL", "Local Ollama endpoint URL.", section="Model", advanced=True),
    OperatorFieldSpec("OLLAMA_NUM_CTX", "Ollama context window.", section="Model", advanced=True),
    OperatorFieldSpec("USE_SEARXNG", "Enable SearXNG search escalation.", section="Search"),
    OperatorFieldSpec("USE_DDGS", "Enable DDGS search fallback.", section="Search"),
    OperatorFieldSpec("USE_TAVILY", "Enable Tavily search fallback.", section="Search"),
    OperatorFieldSpec("SEARXNG_PORT", "SearXNG service port.", section="Services"),
    OperatorFieldSpec("SEARXNG_BASE_URL", "SearXNG endpoint URL.", section="Services", advanced=True),
    OperatorFieldSpec("TAVILY_API_KEY", "Tavily API key.", secret=True, section="Search", advanced=True),
    OperatorFieldSpec("REDIS_HOST", "Redis host.", section="Services"),
    OperatorFieldSpec("REDIS_PORT", "Redis port.", section="Services"),
    OperatorFieldSpec("REDIS_DB", "Redis database number.", section="Services", advanced=True),
    OperatorFieldSpec("REDIS_MAX_CONNECTIONS", "Redis maximum connection count.", section="Services", advanced=True),
    OperatorFieldSpec("REDIS_SOCKET_TIMEOUT", "Redis socket timeout in seconds.", section="Services", advanced=True),
)
