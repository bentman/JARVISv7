...
class SessionService:
    ...
    def record_wake_detection(self) -> WakeMonitorStatus:
        self._wake_status = WakeMonitorStatus(
            provider=self._wake_status.provider,
            available=True,
    ...
    def record_wake_idle(self, reason: str = "wake listening") -> WakeMonitorStatus:
        self._wake_status = WakeMonitorStatus(
            provider=self._wake_status.provider,
            available=True,
            reason=reason,
    ...
    def record_wake_error(self, error: Exception | str, reason: str = "wake detection error; PTT-only fallback is active") -> WakeMonitorStatus:
        self._wake_status = WakeMonitorStatus(
            provider=self._wake_status.provider,
            available=False,
            reason=reason,
    ...
    def process_wake_chunks(self, wake_runtime: WakeRuntime, audio_chunks: Iterable[np.ndarray]) -> WakeMonitorStatus:
        ...
        try:
            ...
            return self.record_wake_error(exc)
